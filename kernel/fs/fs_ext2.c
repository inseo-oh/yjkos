...
NODISCARD static int read_blocks(struct fscontext *self, void *buf, uint32_t block_addr, blkcnt_t blkcount) {
    ...
    DISK_BLOCK_ADDR diskblockaddr = block_addr * (self->blocksize / self->disk->physdisk->blocksize);
...
NODISCARD static int next_triply_block_ptr(uint32_t *addr_out, struct ino_context *self) {
    ...
    tableaddr = uint32_le_at(&self->triply_indirect_buf.buf[self->triply_indirect_buf.offset_in_buf]);
...
NODISCARD static int next_doubly_block_ptr(uint32_t *addr_out, struct ino_context *self) {
    ...
    result_addr = uint32_le_at(&self->doubly_indirect_buf.buf[self->doubly_indirect_buf.offset_in_buf]);
...
NODISCARD static int next_singly_block_ptr(uint32_t *addr_out, struct ino_context *self) {
    ...
    result_addr = uint32_le_at(&self->singly_indirect_buf.buf[self->singly_indirect_buf.offset_in_buf]);
    if (result_addr == 0) {
        goto out;
    }
...
NODISCARD static int openinode(struct ino_context *out, struct fscontext *self, ino_t inode) {
    ...
    for (int i = 0; i < 12; i++) {
        out->direct_block_ptrs[i] = uint32_le_at(&inodedata[0x28 + sizeof(*out->direct_block_ptrs) * i]);
    }
    out->singly_indirect_table = uint32_le_at(&inodedata[0x58]);
...
NODISCARD static ssize_t fd_op_read(struct fd *self, void *buf, size_t len) {
    assert(len <= STREAM_MAX_TRANSFER_SIZE);
...
NODISCARD static ssize_t fd_op_write(struct fd *self, void const *buf, size_t len) {
    assert(len <= STREAM_MAX_TRANSFER_SIZE);
    (void)self;
    (void)buf;
    return -EIO;
...
NODISCARD static int fd_op_seek(struct fd *self, off_t offset, int whence) {
    struct openfdcontext *context = self->data;
...
NODISCARD static int vfs_op_mount(struct vfs_fscontext **out, struct ldisk *disk) {
    ...
    context->blocks_in_block_group = uint32_le_at(&superblk[0x020]);
    context->inodes_in_block_group = uint32_le_at(&superblk[0x028]);
...
static NODISCARD int vfs_op_umount(struct vfs_fscontext *self) {
    heap_free(self->data);
    return 0;
}
...
void fsinit_init_ext2(void) {
    vfs_register_fs_type(&s_fstype, "ext2", &FSTYPE_OPS);
}
