...
static bool do_consume_string_if_match(void) {
    ...
    TEST_EXPECT(smatcher_consume_string_if_match(
        &smatcher, "hello1") == false);
    TEST_EXPECT(smatcher_consume_string_if_match(
        &smatcher, "world")  == false);
    TEST_EXPECT(smatcher_consume_string_if_match(
        &smatcher, "hello")  == true);
    TEST_EXPECT(smatcher.currentindex == 5);
    TEST_EXPECT(smatcher_consume_string_if_match(
        &smatcher, "hello")  == false);
    TEST_EXPECT(smatcher_consume_string_if_match(
        &smatcher, "world")  == false);
    TEST_EXPECT(smatcher_consume_string_if_match(
        &smatcher, " world") == true);
...
static bool do_consume_word_if_match(void) {
    struct smatcher smatcher;
    smatcher_init_with_len(
        &smatcher, "hello worldpeopleguy", 17);
    TEST_EXPECT(smatcher_consume_word_if_match(
        &smatcher, "world")  == false);
    TEST_EXPECT(smatcher_consume_word_if_match(
        &smatcher, "hello")  == true);
    TEST_EXPECT(smatcher.currentindex == 5);
...
static bool do_skip_whitespaces(void) {
    struct smatcher smatcher;
    smatcher_init_with_len(
        &smatcher, "hello    worldpeople", 14);
    TEST_EXPECT(
        smatcher_consume_string_if_match(&smatcher, "hello")
        == true);
    smatcher_skip_whitespaces(&smatcher);
    TEST_EXPECT(smatcher.currentindex == 9);
    TEST_EXPECT(
        smatcher_consume_string_if_match(&smatcher, "world")
        == true);
    TEST_EXPECT(smatcher.currentindex == 14);
    TEST_EXPECT(
        smatcher_consume_string_if_match(&smatcher, "people")
        == false);
...
static bool do_consume_word(void) {
    ...
    smatcher_init_with_len(
        &smatcher, "hello worldpeopleguy", 17);
    TEST_EXPECT(smatcher_consume_word(
        &str, &len, &smatcher) == true);
