...
WARN_UNUSED_RESULT int utf8_getbyteslen(uint8_t leadingbyte) {
    ...
    if ((leadingbyte & 0xe0U) == 0xc0) {
        return 2;
    }
    if ((leadingbyte & 0xf0U) == 0xe0) {
        return 3;
    }
    if ((leadingbyte & 0xf8U) == 0xf0) {
        return 4;
    }
    return -1;
} 
 ...
 */
static int32_t utf8_tocodepoint(uint8_t const *buf) {
    // https://scripts.sil.org/cms/scripts/page.php?id=iws-appendixa&site_id=nrsi
    uint32_t result = 0;

    if (buf[0] < 128) {
        result = buf[0];
    } else if ((buf[0] & 0xe0U) == 0xc0) {
        if ((buf[1] & 0xc0U) != 0x80) {
            goto badcode;
        }
        result = ((uint32_t)(buf[0] & 0x1fU) << 6U) |
                 (uint32_t)(buf[1] & 0x3fU);
    } else if ((buf[0] & 0xf0U) == 0xe0) {
        if (
            ((buf[1] & 0xc0U) != 0x80) ||
            ((buf[2] & 0xc0U) != 0x80)
        ) {
        ...
        result = ((uint32_t)(buf[0] & 0x0fU) << 12U) |
                 ((uint32_t)(buf[1] & 0x3fU) << 6U) |
                 (uint32_t)(buf[2] & 0x3fU);
    } else if ((buf[0] & 0xf8U) == 0xf0) {
        if (
            ((buf[1] & 0xc0U) != 0x80) ||
            ((buf[2] & 0xc0U) != 0x80) ||
            ((buf[3] & 0xc0U) != 0x80)
        ...
        result = ((uint32_t)(buf[0] & 0x07U) << 18U) |
                 ((uint32_t)(buf[1] & 0x3fU) << 12U) |
                 ((uint32_t)(buf[2] & 0x3fU) << 6U) |
...
static void init_psf2(void) {
    ...
    while ((nextchr < _binary_kernelfont_psf_end) && (glyph < numglyph)) {
        ...
        int byteslen = utf8_getbyteslen(byt);
        int32_t unicode = 0;
        if (byteslen < 0) {
            goto badcode;
        }
