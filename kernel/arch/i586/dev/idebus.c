...
static void atadisk_op_issue_cmd(struct atadisk *self, ATA_CMD cmd) {
    struct disk *disk = self->data;
...
static void atadisk_op_read_data(struct ata_data_buf *out, struct atadisk *self) {
    struct disk *disk = self->data;
    io_in16_rep(disk->bus, IOREG_DATA, out->data, sizeof(out->data) / sizeof(*out->data));
}
static void atadisk_op_write_data(struct atadisk *self, struct ata_data_buf *buffer) {
    struct disk *disk = self->data;
...
static bool atadisk_op_dma_begin_session(struct atadisk *self) {
    struct disk *disk = self->data;
...
static void atadisk_op_dma_end_session(struct atadisk *self) {
    struct disk *disk = self->data;
    struct bus *bus = disk->bus;
...
[[nodiscard]] static int atadisk_op_dma_init_transfter(struct atadisk *self, void *buffer, size_t len, bool is_read) {
    struct disk *disk = self->data;
    struct bus *bus = disk->bus;
    assert(bus->busmaster_enabled);
    ...
    for (size_t i = 0; remaining_size != 0; i++) {
        ...
        if (!is_read) {
            pmemcpy_out(bus->prdt[i].buffer_physaddr, &((uint8_t *)buffer)[i * MAX_TRANSFTER_SIZE_PER_PRD], currentsize, true);
        }
...
[[nodiscard]] static int atadisk_op_dma_begin_transfer(struct atadisk *self) {
    struct disk *disk = self->data;
    struct bus *bus = disk->bus;
...
static ATA_DMASTATUS atadisk_op_dma_check_transfer(struct atadisk *self) {
    struct disk *disk = self->data;
    struct bus *bus = disk->bus;
    /* We have to read the status after IRQ. */
    uint8_t bmstatus = busmaster_in8(bus, BUSMASTER_REG_STATUS);
    if (bmstatus & (1U << 1)) {
        uint16_t pcistatus = pci_read_status_reg(bus->pcipath);
        drive_printf(bus, disk->driveid, "DMA error occured. busmaster status %02x, PCI status %04x\n", bmstatus, pcistatus);
        pci_write_status_reg(bus->pcipath, PCI_STATUSFLAG_MASTER_DATA_PARITY_ERROR | PCI_STATUSFLAG_RECEIVED_TARGET_ABORT | PCI_STATUSFLAG_RECEIVED_MASTER_ABORT);
...
static struct atadisk_ops const OPS = {
    .dma_begin_session = atadisk_op_dma_begin_session,
    .dma_end_session = atadisk_op_dma_end_session,
    ...
    .issue_command = atadisk_op_issue_cmd,
...
static bool init_busmaster(struct bus *bus) {
    ...
    for (size_t i = 0; i < bus->prd_count; i++) {
        ...
        size_t current_page_count = size_to_blocks(current_size, ARCH_PAGESIZE);
...
static int init_controller(struct shared *shared, uint16_t io_base, uint16_t ctrl_base, uint16_t busmastrer_base, PCIPATH pcipath, bool busmaster_enabled, uint8_t irq, size_t channel_index) {
    ...
    archi586_pic_register_handler(&bus->irq_handler, irq, irq_handler, bus);
...
static void reprogram_progif(PCIPATH path, uint8_t progif) {
    ...
    if (progif != new_progif) {
        ...
        pci_write_progif(path, new_progif);
        progif = pci_read_progif(path);
        if (progif != new_progif) {
...
static void pci_probe_callback(PCIPATH path, uint16_t venid, uint16_t devid, uint8_t baseclass, uint8_t subclass, void *data) {
    ...
    uint8_t progif = pci_read_progif(path);
