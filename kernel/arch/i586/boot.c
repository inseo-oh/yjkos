...
static void init_serial0(void) {
    ...
    co_set_debug_console(&s_serial0.tty.stream);
...
static void init_serial1(void) {
    ...
    archi586_serial_use_irq(&s_serial1);
    ret = archi586_serial_init_iodev(&s_serial1);
    if (ret < 0) {
...
[[noreturn]] void archi586_init(uint32_t mb_magic, PHYSPTR mb_info_addr) {
    if (CONFIG_EARLY_VGATTY) {
        archi586_vgatty_init_early_debug();
    }
    ...
    archi586_gdt_init();
    archi586_idt_init();
    arch_mmu_write_protect_after_early_init();
    ...
    if (s_serial0_ready) {
        archi586_serial_use_irq(&s_serial0);
    }
    init_serial1();
