...
static void ioout8(bus_t *self, ioreg_t reg, uint8_t data) {
    archx86_out8(self->iobase + reg, data);
}
static void ioout16(bus_t *self, ioreg_t reg, uint16_t data) {
    archx86_out16(self->iobase + reg, data);
}
static uint8_t ioin8(bus_t *self, ioreg_t reg) {
    return archx86_in8(self->iobase + reg);
}
static uint16_t ioin16(bus_t *self, ioreg_t reg) {
    return archx86_in16(self->iobase + reg);
}
static void ioin16rep(bus_t *self, ioreg_t reg, void *buf, size_t len) {
...
static void ctrlout8(bus_t *self, ctrlreg_t reg, uint8_t data) {
    archx86_out8(self->ctrlbase + reg, data);
}
static void ctrlout16(bus_t *self, ioreg_t reg, uint16_t data) {
    archx86_out16(self->ctrlbase + reg, data);
}
static uint8_t ctrlin8(bus_t *self, ctrlreg_t reg) {
    return archx86_in8(self->ctrlbase + reg);
}
static uint16_t ctrlin16(bus_t *self, ctrlreg_t reg) {
    return archx86_in16(self->ctrlbase + reg);
}

static ata_status_t readstatus(bus_t *self) {
    return ctrlin8(self, CTRLREG_ALTERNATESTATUS);
}

static void busprintf(bus_t *self, char const *fmt, ...) {
    va_list ap;
...
static void driveprintf(bus_t *self, int drive, char const *fmt, ...) {
    va_list ap;
...
static void resetbus(bus_t *self) {
...
static FAILABLE_FUNCTION selectdrive(bus_t *self, int8_t drive) {
FAILABLE_PROLOGUE
...
static void atadisk_op_softreset(atadisk_t *self) {
    disk_t *disk = self->data;
    resetbus(disk->bus);
}
static FAILABLE_FUNCTION atadisk_op_selectdisk(atadisk_t *self) {
    disk_t *disk = self->data;
    return selectdrive(disk->bus, disk->driveid);
}
static ata_status_t atadisk_op_readstatus(atadisk_t *self) {
    disk_t *disk = self->data;
    return readstatus(disk->bus);
}
static void atadisk_op_setfeaturesparam(atadisk_t *self, uint16_t data) {
    disk_t *disk = self->data;
    ioout8(disk->bus, IOREG_FEATURES, data);
}
static void atadisk_op_setcountparam(atadisk_t *self, uint16_t data) {
    disk_t *disk = self->data;
    ioout8(disk->bus, IOREG_SECTORCOUNT, data);
}
static void atadisk_op_setlbaparam(atadisk_t *self, uint32_t data) {
    disk_t *disk = self->data;
...
static void atadisk_op_setdeviceparam(atadisk_t *self, uint8_t data) {
    disk_t *disk = self->data;
    uint8_t regvalue = ioin8(disk->bus, IOREG_DRIVE_AND_HEAD);
...
static uint32_t atadisk_op_getlbaoutput(atadisk_t *self) {
    disk_t *disk = self->data;
...
static void atadisk_op_issuecmd(atadisk_t *self, atacmd_t cmd) {
    disk_t *disk = self->data;
    ioout8(disk->bus, IOREG_COMMAND, cmd);
}
static bool atadisk_op_getirqflag(atadisk_t *self) {
    disk_t *disk = self->data;
    return disk->bus->gotirq;
}
static void atadisk_op_clearirqflag(atadisk_t *self) {
    disk_t *disk = self->data;
    disk->bus->gotirq = false;
}
static void atadisk_op_readdata(ata_databuf_t *out, atadisk_t *self) {
    disk_t *disk = self->data;
    ioin16rep(disk->bus, IOREG_DATA, out->data, sizeof(out->data)/sizeof(*out->data));
}
static void atadisk_op_writedata(atadisk_t *self, ata_databuf_t *buffer) {
    disk_t *disk = self->data;
...
static void busmasterout8(bus_t *self, busmasterreg_t reg, uint8_t data) {
    assert(self->busmasterenabled);
    archx86_out8(self->busmastrerbase + reg, data);
}
static void busmasterout32(bus_t *self, busmasterreg_t reg, uint32_t data) {
    assert(self->busmasterenabled);
    archx86_out32(self->busmastrerbase + reg, data);
}
static uint8_t busmasterin8(bus_t *self, busmasterreg_t reg) {
    assert(self->busmasterenabled);
    return archx86_in8(self->busmastrerbase + reg);
}
static uint32_t busmasterin32(bus_t *self, busmasterreg_t reg) {
    assert(self->busmasterenabled);
...
static bool atadisk_op_dma_beginsession(atadisk_t *self) {
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
...
static void atadisk_op_dma_endsession(atadisk_t *self) {
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
...
static void atadisk_op_lock(atadisk_t *self) {
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
    bool expected = false;
    bool desired = true;
...
static void atadisk_op_unlock(atadisk_t *self) {
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
    bool desired = false;
...
static FAILABLE_FUNCTION atadisk_op_dma_inittransfter(atadisk_t *self, void *buffer, size_t len, bool isread) {
FAILABLE_PROLOGUE
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
    assert(bus->busmasterenabled);
...
static FAILABLE_FUNCTION atadisk_op_dma_begintransfer(atadisk_t *self) {
FAILABLE_PROLOGUE
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
...
static ata_dmastatus_t atadisk_op_dma_checktransfer(atadisk_t *self) {
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
...
static void atadisk_op_dma_endtransfer(atadisk_t *self, bool wassuccess) {
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
    busmasterout8(bus, BUSMASTERREG_CMD, busmasterin8(bus, BUSMASTERREG_CMD) & ~BUSMASTER_CMDFLAG_START);
...
static void atadisk_op_dma_deinittransfer(atadisk_t *self) {
    disk_t *disk = self->data;
    bus_t *bus = disk->bus;
    assert(bus->busmasterenabled);
}

static atadisk_ops_t const OPS = {
...
static FAILABLE_FUNCTION initcontroller(shared_t *shared, archx86_ioaddr_t iobase, archx86_ioaddr_t ctrlbase, archx86_ioaddr_t busmastrerbase, pcipath_t pcipath, bool busmasterenabled, uint8_t irq, size_t channalindex) {
FAILABLE_PROLOGUE
    bus_t *bus = heap_alloc(sizeof(*bus), HEAP_FLAG_ZEROMEMORY);
    if (bus == NULL) {
        THROW(ERR_NOMEM);
    }
    bus->shared = shared;
...
cont:
    ...
    for (int8_t drive = 0; drive < 2; drive++) {
        ...
        disk_t *disk = heap_alloc(sizeof(*disk), HEAP_FLAG_ZEROMEMORY);
        if (disk == NULL) {
            continue;
        }
        disk->bus = bus;
...
doinit:
    ...
    shared_t *shared = heap_alloc(sizeof(*shared), HEAP_FLAG_ZEROMEMORY);
    if (shared == NULL) {
