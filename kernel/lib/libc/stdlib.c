...
int rand(void) {
    uint32_t out = 0;
    for (int i = 0; i < 32; i++) {
        uint16_t t1 = s_randlfsr >> 0U;
        uint16_t t2 = s_randlfsr >> 2U;
        uint16_t t3 = s_randlfsr >> 3U;
        uint16_t t4 = s_randlfsr >> 5U;
        uint16_t xor = (t1 ^ t2 ^ t3 ^ t4) & 0x1U;
        uint16_t shiftedbitout = s_randlfsr & 0x1U;
        s_randlfsr = (s_randlfsr >> 1U) | (xor << 15U);
